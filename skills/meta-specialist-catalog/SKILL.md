---
name: meta-specialist-catalog
description: >
  Use this normal Agent Skill first for specialized topics, rare tools and direct lookup of a named legacy skill. Decompose the request and select concrete atomic skills using ordinary file search and its child catalog. Typical requests: rare integrations, explicitly named original skills, unmatched topics.
---

# meta-specialist-catalog
...
4. For a separate subtask in another domain, search `../` (the first-level `skills/` directory) again for another meta-skill. For an explicit original skill name whose domain is unclear, read `references/legacy-names.md`.
...
The short description in `references/members.md` is a navigation entry, **not** the complete skill. **Use the existing file-search tool on the exact path** copied from that catalog (or `references/legacy-names.md` for an exact-name lookup), for example `../../atomic-skills/<selected-child>/SKILL.md`. Open/read the **entire** matched `SKILL.md`: a search-result snippet alone is insufficient.
...
- rare integrations: select a matching Start skill from `references/members.md`; add Support or Check only if the task requires it.
- explicitly named original skills: select a matching Start skill from `references/members.md`; add Support or Check only if the task requires it.
- unmatched topics: select a matching Start skill from `references/members.md`; add Support or Check only if the task requires it.
