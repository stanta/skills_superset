...
description: >
  Use this normal Agent Skill first for genomics, NGS, genetics, transcriptomics and biomedical datasets. Decompose the request and select concrete atomic skills using ordinary file search and its child catalog. Typical requests: NGS workflows, variant interpretation, gene expression. Russian queries: геномика; секвенирование.
---
...
- NGS workflows: select a matching Start skill from `references/members.md`; add Support or Check only if the task requires it.
- variant interpretation: select a matching Start skill from `references/members.md`; add Support or Check only if the task requires it.
- gene expression: select a matching Start skill from `references/members.md`; add Support or Check only if the task requires it.
