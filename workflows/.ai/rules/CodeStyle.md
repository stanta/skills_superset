...
- Не добавляйте оператор `use` для встроенных типов и функций PHP/Python/TS (int, string, count(), etc.)
...
4. **При работе с Entity и DTO:**
   - Использовать публичные readonly свойства вместо геттеров (согласно возможностям PHP/Python/TS 8.5)
...
- Всегда добавляй `declare(strict_types=1);` в файлы с бизнес-логикой.
- Соблюдай PSR-12 и единый стиль форматирования (php-cs-fixer/PHPCS).
- Используй строгие типы и избегай `mixed` в публичных API.
- Исключения домена должны быть осмысленными и конкретными, без «Exception» по умолчанию.
- Следи за N+1 запросами: используй eager loading там, где это уместно.

### Python

- Используй PEP 8 и автоформаттеры (black/isort) для стабильного diffs.
- Включай строгую типизацию (mypy/pyright) и предпочитай `typing` вместо неявных типов.
- Для публичных функций пиши docstrings по PEP 257, описывай исключения.
- Не смешивай синхронный и асинхронный код; для async используй `asyncio`-совместимые библиотеки.
- Минимизируй побочные эффекты: функции должны быть предсказуемыми и тестируемыми.

### TypeScript

- Включай `strict` режим и не используй `any` без обоснования.
- Явно разделяй типы данных и бизнес-логику: DTO/Schema — отдельно от домена.
- Используй `type-only imports` и `import type` для оптимизации бандла.
- Старайся использовать `unknown` вместо `any` и валидируй входные данные.
- Следи за совместимостью с ESLint/Prettier и единым стилем в проекте.
