...
### PHPStan-mypy-lint выдает ошибки после работы агента
...
Агент `PHPStan-mypy-lint-developer` автоматически исправляет ошибки. Если не помогло:

**PHP:**

1. Посмотрите ошибки: `make php-run CMD="vendor/bin/PHPStan-mypy-lint analyse --level=8"`
2. Запустите агента повторно с контекстом ошибок
3. Если ошибки сложные, исправьте вручную

**Python:**

1. Посмотрите ошибки: `make python-run CMD="python -m mypy --strict ."`
2. Запустите агента повторно с контекстом ошибок
3. Если ошибки сложные, исправьте вручную

**TypeScript:**

1. Посмотрите ошибки: `make ts-run CMD="npx tsc --noEmit --strict"`
2. Запустите агента повторно с контекстом ошибок
3. Если ошибки сложные, исправьте вручную
...
1. Запустите тесты: `make python-run CMD="python -m pytest"`
2. Изучите ошибки
3. Если проблема в коде - вернитесь к Шагу 5 (разработка)
4. Если проблема в тестах - вернитесь к Шагу 6 (тестирование)
5. Укажите агенту конкретные ошибки для исправления
...
1. Запустите тесты: `make ts-run CMD="npm test"`
2. Изучите ошибки
3. Если проблема в коде - вернитесь к Шагу 5 (разработка)
4. Если проблема в тестах - вернитесь к Шагу 6 (тестирование)
5. Укажите агенту конкретные ошибки для исправления
...
### Что такое модульный монолит и когда применять микросервис?

**Модульный монолит ** - это решение :
...
**микросервисы - это решение :**

- Набор небольших независимых сервисов (каждый отвечает за отдельную бизнес-возможность)
- Каждый сервис можно разворачивать (deploy) и масштабировать независимо
- Взаимодействие между сервисами — через API (HTTP/gRPC) и/или брокер сообщений
- Данные по возможности изолированы: у сервиса своя БД/схема (или строгие границы владения данными)

**Когда применять**:

- Есть реальная потребность в независимых релизах (разные части продукта должны выкатываться отдельно)
- Нужна разная нагрузочная модель (один домен требует масштабирования, другие — нет)
- Несколько команд работают параллельно и им нужны четкие границы владения
- Требования по надежности/изоляции сбоев выше, чем может дать монолит

**Цена и риски** (что усложняется):

- Инфраструктура и эксплуатация (CI/CD, сервис-дискавери, мониторинг/трейсинг, управление конфигурацией)
- Сквозное тестирование (end-to-end) и отладка распределенных проблем
- Согласованность данных и транзакции (часто нужны паттерны saga/outbox, идемпотентность)

**Практическое правило**:

- Начинайте с модульного монолита и переходите к микросервисам, когда преимущества (независимые релизы/масштабирование/команды) начинают окупать операционную сложность.
...
1. Проверьте [основную документацию](../full-project.md)
