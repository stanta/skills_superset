...
4. Агент `PHPStan-mypy-lint-developer` проверяет типы
5. Агент `phpcs-developer` проверяет стиль кода

**Результат**: Созданы файлы PHP/Python/TS в `/backend/src/Currency/`
...
5. Агент `PHPStan-mypy-lint-developer` проверяет типы
...
- [ ] PHPStan-mypy-lint не выдает ошибок
...
- [ ] Покрытие кода ≥ 75%
- [ ] PHPStan-mypy-lint и PHPCS не выдают ошибок
- [ ] Коммит сделан
...
### Проблема: PHPStan-mypy-lint выдает много ошибок

**Причина**: Агент не учел все типы.

**Решение**: Агент `PHPStan-mypy-lint-developer` автоматически исправит. Если не помогло, запустите повторно.
